  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end justify-center p-0 md:items-center md:p-4">
      <button
        ...
        aria-hidden="true"
        tabIndex={-1}
        // El fondo se oscurece Y se desenfoca un poco. Oscurecer solo deja la
        // pantalla de atras perfectamente legible compitiendo con el dialogo.
        // Un desenfoque corto la empuja hacia atras sin ocultarla: se sigue
        // viendo DONDE estabas —no es una pantalla nueva, es una capa
        // encima— pero ya no se puede leer, asi que la atencion cae sola en
        // la ventana. Es la diferencia entre tapar y enfocar.
      ...
      <section
        ...
        tabIndex={-1}
        className={cn(
          'relative flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-2xl bg-white shadow-[var(--sombra-modal)] md:rounded-2xl',
          sizes[size],
        )}
      >
        {title ? (
          <header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 id={titleId} className="text-lg font-semibold tracking-[-0.018em] text-brand-950">{title}</h2>
                {description ? <p id={descriptionId} className="mt-1 text-sm leading-[1.65] text-slate-600">{description}</p> : null}
              </div>
            </div>
